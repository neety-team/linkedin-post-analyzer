...
**⛔ LA ALARGADA SOLO TIENE DOS SITIOS, Y ROTA (Iker, 2026-09-18):** *"repites mucho claro con varias oes… y las colocas en posiciones que no tienen sentido… una frase, bueno con tres oes, y luego otra frase. Suena fatal"*. Sitios que valen: **(1) la primera palabra** de la respuesta (`siii, …`, `clarooo que…`) o **(2) dentro de la primera frase, entre sus tres primeras palabras y justo antes de una coma** (`pues siii, …`). Cualquier otra posición la devuelve el código a su forma normal (`sitioAlargable`) y, si toca alargar, abre con la palabra sorteada. El banco pasa de 5 a 13 palabras (`claro, si, bueno, bien, vale, genial, total, justo, exacto, cierto, perfecto, brutal, tal cual`), y **en la misma tanda no se repite ninguna de las 4 últimas** (`ALARGADAS_POR_POST`). Vale igual en Google Chat.
