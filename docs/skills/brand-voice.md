...
- **Emojis SÍ, pero con criterio.** Si el comentario es SOLO un nombre (alguien etiqueta a un colega) o SOLO emojis/una reacción sin palabras → responde con UN único emoji de apoyo que encaje (🙌 · 🔥 · 💪 · 👏 · ❤️ · 😄) y **sin frases**. Fuera de ese caso, el emoji es opcional y sobrio.
  - **⛔⛔ PERO EL NOMBRE NO SE QUITA NUNCA (Iker, 2026-09-23).** Aquí ponía *"sin nombre ni frases"* y estaba mal: la respuesta a un comentario de solo emojis sale **`Nombre Apellido 🙌`**, con el nombre verbatim en la posición 0 y un espacio. **Es lo único que LinkedIn convierte en @-mención**, así que sin él le contestas a alguien sin etiquetarle, y esa persona no se entera. El caso: el 23/09 la herramienta devolvió `🙌` a secas a un comentario de solo emojis y el panel marcaba `@Erica Fernandez Higueras ✗`.
  - **Y con una PÁGINA DE EMPRESA, igual:** el chip no existe (LinkedIn no deja etiquetar una página, Unipile da 422), pero **el nombre abre la respuesta igual, como texto**. Lo único que cambia es que no queda en azul.
  - **Mecanizado** en `replyGenerator.ts` (`respuestaDeApoyo`, RULE 11) y en el atajo de `routes/accounts.ts` para comentarios sin letras (GIF, imagen o solo emojis), con su prueba en `backend/src/scripts/probarRespuestaEmoji.ts`. El panel enseña ahora `Empresa ✓ sin @` cuando el que comenta es una página.
...
- **⛔⛔ Y NUNCA DOS PUNTOS `:` EN UNA RESPUESTA NI EN UN COMENTARIO (Iker, 2026-08-12).** Es de la misma familia que el guion largo. Nadie que conteste un comentario desde el móvil monta una cláusula y luego anuncia el resto con dos puntos, pero el modelo lo hace todo el rato. **Se cambia por una coma, o se parte en dos frases cortas.** ❌ *"Lo caro no es eso: es dar con el que decide"* · ✅ *"Lo caro no es eso, es dar con el que decide"*.
  ...
  - **🔴 Y EN UN POST, LA LÍNEA QUE ABRE UNA LISTA TAMPOCO ACABA EN DOS PUNTOS (Iker, 2026-09-23).** *"Mañana lo contamos en una sala:"* delante de un bloque de 3 → *"no me gusta nada, creo que nunca lo hemos hecho, pon punto final"*. Se cierra con punto y el bloque de debajo se lee solo. Los únicos `:` de final de línea que quedan en un post son el del enlace del ninja y la entrada al bloque de menciones del peloteo. **Mecanizado** en `validar-post.py` (`Ninguna linea termina en dos puntos`): fallo duro en historia, aviso en el resto.
...
**⛔ LA ALARGADA SOLO TIENE DOS SITIOS, Y ROTA (Iker, 2026-09-18):** *"repites mucho claro con varias oes… y las colocas en posiciones que no tienen sentido… una frase, bueno con tres oes, y luego otra frase. Suena fatal"*. Sitios que valen: **(1) la primera palabra** de la respuesta, con pausa detrás (`siii, …`; de corrido solo `clarooo que…`, `graciaas por…`, `muuuy…`) o **(2) justo antes de la primera coma, detrás de una muletilla** (`pues siii, …`, `uff clarooo, …`). La prueba contra producción del 17/09 cazó los dos casos que no: `perfectooo y la excusa…` (sin pausa) y `apúntate ciertooo, que…` (detrás de un verbo). Y si el asentimiento sorteado es alargable, es la misma palabra: salió `tal cuaaal, te compro eso, tal cual`. Cualquier otra posición la devuelve el código a su forma normal (`sitioAlargable`) y, si toca alargar, abre con la palabra sorteada. El banco pasa de 5 a 13 palabras (`claro, si, bueno, bien, vale, genial, total, justo, exacto, cierto, perfecto, brutal, tal cual`), y **en la misma tanda no se repite ninguna de las 4 últimas** (`ALARGADAS_POR_POST`). Vale igual en Google Chat.

**⛔ UNA SOLA PALABRA DE ASENTIR, Y NUNCA COMO INCISO (Iker, 2026-09-18).** La misma prueba sacó `exactooo, claro y tanto, aunque…`, `pues geniaaal, el filtro, tal cual, siempre…` y `justooo, el interlocutor bien, que…`. La causa: al modelo le llegaban DOS palabras sorteadas (la de asentir y la alargada) más el arranque, y las encajaba donde podía. Ahora, si toca alargar, **la alargada es el asentimiento** y no se suma otro. `problemaDeEstilo` reintenta si hay dos asentimientos seguidos, un `, tal cual,` / `, claro,` suelto en mitad o una alargada fuera de sitio, y si a la tercera sigue, `quitarIncisosSueltos` quita el inciso entero con su coma en vez de dejarlo huérfano. `bien`, `vale` y `total` no cuentan como inciso (`está bien, pero` es castellano normal).
...
**⛔⛔ Y LA APERTURA SE ASIGNA, NO SE OFRECE (Iker, 2026-09-15).** El bloque de arriba pedía "cinco ángulos obligatoriamente distintos" **en forma de MENÚ**, y un menú no produce variedad: el modelo los colapsa porque nadie le dice cuál va en cuál. **Los 19 comentarios que medí de este generador salen de ahí: 15 abren con una abstracción y `la eficiencia` y `la diferencia` salen dos veces cada una.**
- **Ahora el ángulo Y el arranque van NUMERADOS y asignados** por llamada, sorteados **sin reemplazo** de un banco de 7 ángulos y 8 arranques. Que el banco tenga más entradas que huecos es lo que hace que roten **también entre posts**: con exactamente cinco, dos tandas distintas vuelven a los mismos cinco.
- **Y se comprueba la salida**, que es lo único que ha funcionado siempre aquí: si alguno abre con una fórmula de IA, o si **dos empiezan con las mismas dos palabras**, se vuelve a pedir la tanda entera con el fallo delante. Lo segundo es lo que de verdad importa en este pilar: los pega **gente distinta en el mismo hilo**, y dos aperturas iguales delatan la coordinación entera.
- **Dos intentos, no tres**, porque genera cinco de golpe y cuesta cinco veces más que una respuesta. Si el segundo sigue flojo, se devuelve con un aviso en el log: los pega una persona que puede editarlos.
