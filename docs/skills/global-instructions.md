...
| quitar `sala` de la 2 por repetida | ✅ **corregido por Iker el mismo día** | yo la mantuve por `§4.4b-ANTECEDENTE` y era la salida mala: la ambigüedad se quita **cambiando el sujeto**, no repitiendo el sustantivo. `Nosotros sí` no tiene antecedente que dudar, contrapone persona a objeto (`una pieza no` / `nosotros sí`) y el `sí` recupera el verbo entero. **Si el usuario dice que algo le sobra, se busca la forma sin ello; la regla del antecedente prohíbe la duda, no obliga a repetir** |

**Línea final: `Nosotros sí, pero ya solo quedan 15 plazas:`** — el `ya` dice que se están llenando (prueba social sin dar la cifra de inscritos) y el `pero` pone la pérdida. **Lo que faltaba de verdad era el `sí`** (`§4.4b-EXCEPCIÓN`): `Solo quedan 15 puestos en esa sala` informaba; `En esa sala sí, pero solo quedan 15 plazas` da la vuelta a la norma y el `pero` convierte la cifra en **lo que puedes perder** (aversión a la pérdida), que empuja más que la orden y sin sonar a anuncio. **Mecanizado:** aviso `la linea del enlace no ORDENA ni parte en dos frases`.
...
2. **Saturar con clichés locales** (8 o más tejidos por el cuerpo, `post-workflow §4.0d` punto 4; no uno arriba y olvidado). Cada referencia que un local reconoce = un "ostras, es de los míos" = trigger de comentario. "Sin forzar": dos bien tejidos baten a cinco grapados.
...
4. **Menciones @:** 🔄 **manda `post-workflow §4.0d` punto 3** (cargo con poder de decisión, actividad de cualquier tipo y empresa con persona antes que sin ella). Lo que sigue es la regla VIEJA del 2026-07-24, revertida el 26/08: solo personas que **hayan COMENTADO en el post de OTRO en los últimos 3 meses**. No basta con publicar o compartir en su propio perfil: el que solo emite y nunca comenta a nadie casi nunca nos comenta, y la mención es alcance tirado. Se comprueba con `GET {BASE}/api/v1/users/{provider_id}/comments?account_id=…`. Ordena por recencia del último comentario. Descartar a quien no comente aunque sea el CEO — un mando que comenta bate a un CEO que solo se autopublica. Además, para el CSV, se saca el **logo de LinkedIn** de cada empresa vía Unipile (no favicons).
...
3. **La línea del enlace resuelve exactamente lo que la de arriba dice que falta**, y ahí ya se habla de identificación, **con el complemento puesto** (`§4.4b-EXPLICITO`): *"Te damos el nombre del que compra dentro, con su contacto: {link}"*. ⛔ **`con su contacto`, no `con su teléfono` (Iker, 2026-09-16)**: no está confirmado que la herramienta entregue el móvil de cada persona (`§4.4b-PROMESA`), y ofrecer el teléfono de un tercero es la familia de datos de terceros de `brand-voice §2c-DATOS`. Ojo, `teléfono` NO tiene ningún capado detrás: está en nuestros mayores posts (168.926 · 192.303), pero como el teléfono **del comercial**, no el de otra persona que nosotros entregamos. 🔴 **Aquí ponía `"Ese nombre lo ponemos nosotros"` y era exactamente el fallo que `§4.4b-EXPLICITO` prohíbe**: la receta enseñaba la elipsis en su propio ejemplo.
...
>    - 🔎 **Leído en el informe del 14/09 el 23/09, y acota la hipótesis:** las tres citas literales de *llegar tarde* (**Biele Group, ICP A**: *"cuando un proyecto se hace público, está adjudicado ya… normalmente vas tarde"* · **Airlan, ICP A**: *"los proyectos privados no se publican en ningún sitio… cuando se visa ya es tarde"* · Ecenarro: *"tres, cuatro años antes"*) están archivadas como **objeción a las señales**, no como dolor que les resolvemos. O sea: el dolor es real, pero **con `/agendar/` la línea 2 no puede prometer "llegas antes"**, porque es justo lo que objetan de las señales públicas. **Donde sí pasa `§4.4b-PROMESA` es con la puerta del evento**: conocer en persona a quien firma antes de que el proyecto salga en prensa. Primer uso: historia de Iker del 23/09.
