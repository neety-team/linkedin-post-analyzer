...
- **⭐⭐⭐ CÓMO SE INVENTA EL CONCEPTO — LA FÓRMULA, que hasta el 2026-08-03 no estaba escrita.** Aquí ponía *"inventa uno original y geográfico"*, y eso no es un método, es un deseo. Por ese hueco entregué `el pasillo de España` (que además pincha al país) y `la región más extensa de la Unión Europea` (que es un DATO, no una metáfora). Iker: *"me parece una basura… trastienda del norte era superoriginal"*.
  ...
  - **EL PROCEDIMIENTO, cuatro pasos y en este orden:**
    ...
    3. **Comprueba las dos cosas que lo tumban:** ¿la palabra la usa un señor de 55 años en un bar? ¿lo usó ESTA cuenta en sus 3 últimos peloteos, aunque sea a medias (`CONCEPTO_QUEMADO`, `global §2.0b-VENTANA`)?
...
- **Concepto: DERIVADO DE LA GEOGRAFÍA de la región** (su posición en el mapa, accidentes geográficos, fronteras). Nunca "región/pueblo/tierra". Así lo hemos hecho siempre: Galicia = "la esquina del Atlántico" (arriba a la izquierda), Álava/País Vasco = "la trastienda del norte" (arriba, la olvidada del norte), Navarra = "el patio trasero de los Pirineos" (frontera con Francia). Inventa uno original y geográfico por región; no repitas uno que esta cuenta haya usado en sus 3 últimos peloteos (`global §2.0b-VENTANA`).
...
- **⭐⭐ EL CONCEPTO TAMBIÉN VA EN PALABRA UNIVERSAL, no solo los clichés (Iker, 2026-07-29).** Aquí solo se pedía que fueran universales los 2 clichés, y por ese hueco colé *"un páramo con catedrales"*: **"páramo" no lo usa medio España**. El concepto lo lee TODO el mundo antes de pulsar "ver más", así que **una palabra rara ahí sale más cara que en ningún otro sitio del post**.
  ...
  - **El test:** ¿la usaría un señor de 55 años hablando en un bar? Si dudas, no vale. Literario, técnico o de diccionario = fuera. Y sigue aplicando lo de siempre: **el concepto no se repite en los 3 peloteos siguientes de esa cuenta** (`global §2.0b-VENTANA`; aquí ponía "NUNCA, ni entre cuentas", superado el 16/09).
- **⭐ Clichés del GANCHO: EXACTAMENTE los 2 más UNIVERSALES que tenga la región. Los específicos van al CUERPO** (2026-07-17). **Máximo 2, no 3 (Iker, 2026-07-22).**
  ...
  - **🔄 LOS CLICHÉS NO SE QUEMAN (Iker, 2026-09-16, `global §2.0b-VENTANA` punto 3).** Son de la región, no del post: si la región se vuelve a hacer, sus clichés típicos valen otra vez, y una palabra-cliché compartida por dos regiones también. Lo único que se cuida es no abrir **dos peloteos seguidos de la misma cuenta** con la misma palabra en el gancho. 🔴 Aquí ponía *"NO repitas una palabra-cliché ya usada en otro mapa: playa ya se gastó en Valencia, por eso se cayó de Murcia"*: eran dos mapas de Iker con cuatro peloteos suyos en medio, así que con la regla de hoy `playa` era válida en Murcia.
...
- **⭐ EL VERBO DEL PREJUICIO ROTA SIEMPRE (Iker, 2026-07-31).** El **sujeto ajeno** es obligatorio y no se toca (`la ven`, `nadie habla de`, `la tienen`), pero **el VERBO que lo acompaña se gasta y hay que cambiarlo cada vez**. Pasó con `fichada`: la estrené el 30/07 en el despiece de Euskadi (*"nadie lo tiene fichado como tierra de coches"*), me gustó porque pega mucho más que `la ven`, y al día siguiente abrí el mapa de Asturias con *"la tienen fichada"* **y encima la repetí otra vez tres líneas más abajo**. El verbo es lo PRIMERO que se lee: repetirlo convierte el pilar en plantilla y el lector deja de notar el desprecio, que es el motor entero. **No es que el verbo sea malo, es que ya está gastado, y eso es peor.**
  ...
  - **Mecanizado:** `VERBO_PREJUICIO_QUEMADO` en `scripts/validar-post.py`. **Cuando publiques un peloteo, mete ahí el verbo que hayas usado, con su fecha y su cuenta.** Es fallo duro solo para esa cuenta y durante sus 3 peloteos siguientes (42 días); después el verbo vuelve a estar libre (`global §2.0b-VENTANA`). ⚠️ *"No es que el verbo sea malo, es que ya está gastado"* (arriba) se refiere a eso: a repetirlo en el peloteo siguiente, como pasó con `fichada` al día siguiente.
...
- **🧪 EXPERIMENTO MAPA → LUMA (Iker, 2026-09-22, caduca el 24/09):** el mapa de Álava de Iker lleva como **única puerta** el enlace de Luma en lugar de `/mapas/{region}/`, para probar lo que pide `§4.4-CONVERSION-EVENTO` punto 2 (el post que nombra a gente real de la región es el que mejor vende el evento). Va solo, sin el enlace del mapa, para que los inscritos por 1.000 impresiones se lean limpios; el ninja es de tipo PERSONA (`no te presenta a nadie`), que además sirve a la hipótesis persona contra sala. El validador lo acepta hasta el 24/09. **Se mide contra el 0,51 del "Los 10" de Gipuzkoa.**
...
- **Título — plantilla FIJA, no la reinventes:** `[Región]: el músculo industrial`. Ej.: `Álava: el músculo industrial` · `Aragón: el músculo industrial`. Aquí no toca creatividad: es el formato de la web y se repite mapa a mapa.
  - **Ese título va en PamPam, en el `<title>` y en el og de la web. El H1 de la página NO lo repite (Iker, 2026-09-17):** el H1 es `El <em>mapa industrial</em> de [Región]:` (con `de la` / `del` donde toque) y la **entrada con ángulo va detrás, en la MISMA línea**. Si no cabe, se acorta la entrada, no se parte la línea. Referencia: título + entrada ≈ **115 caracteres como mucho**, y lo que manda es `qa-mapas` (Paso 9b).
...
- **🗺️ EL DISEÑO DE LA PÁGINA (Iker, 2026-09-17) — no se toca al clonar, se comprueba:** cabecera en una línea, página a 1200px, mapa entero en la primera pantalla con aire debajo (mínimo 510px: por debajo PamPam se pone en modo compacto; nunca pegado al borde, porque la rueda sobre el mapa no baja la página) y las cifras debajo con el efecto de scroll. La doctrina y el porqué están en `neety-resources/docs/lead-magnet-playbook.md §5 TER`; el caso, en `lead-magnet-web §4c-bis`.
  - **Antes de dar la página por buena:** `node tools/qa-mapas.cjs <puerto> {slug}` en `neety-resources` (con el estático levantado). Mide a 1532×690 (portátil al 125%, la ventana real de Iker), 1920×960 y móvil. Sale 1 si algo incumple.
  - **Cantabria es la página modelo del diseño.** Un retoque se hace en Cantabria y se reparte con `python tools/mapa-layout-ancho.py mapas/*/index.html`. Clonar Murcia sigue valiendo: es idéntica.
...
- **📌 CÓMO ESTÁN HECHAS LAS PÁGINAS DE LOS MAPAS (respuesta del programador, 2026-07-31 — esto ya no se pregunta más):**
  ...
  - Las **10 páginas de hoy comparten estructura idéntica** (~~cabecera a dos columnas con el mapa a toda altura~~ → desde el 17/09, cabecera en una línea y mapa a todo el ancho, ver el diseño arriba) y solo cambian **~15 líneas**: título/SEO/OG/canonical, H1, lede, las 3 cifras con fuente, el iframe y el párrafo de valor.
...
- **El CONCEPTO no se repite en los 42 días siguientes EN ESA CUENTA** (`§4.0d` punto 7, `global §2.0b-VENTANA`; antes ponía "NUNCA" y después "tampoco entre cuentas"). ⚠️ El "país inventado" 7.87x → 1.2x **no prueba** que repetir mate (el segundo cayó por no ofender a nadie; la nota está en `global-instructions`, junto a la regla del concepto de zona). Si una cuenta hace una región que ya hizo otra, cambia **lo inventado** (concepto, país, frase-rabia); **los clichés de la región pueden repetirse**.
...
- ≥2 semanas desde el último mapa de esa cuenta. **Región: no repetir DENTRO de esa cuenta** (otra cuenta sí puede hacerla). **Concepto: no se repite en los 3 peloteos siguientes de esa cuenta** (`global §2.0b-VENTANA`).
...
- **Concepto: no se repite en los 3 peloteos siguientes de esa cuenta** (`global §2.0b-VENTANA`). **Personas: no se repiten nunca, en ninguna cuenta**, y eso no es una quemada sino negocio: el motor de "Los 10" es llegar a gente nueva (`§4.0c`). Jamás menciones dos veces a la misma persona en dos "Los 10".
...
1. **En la cuenta que va a publicar.** Si esa cuenta la usó en los **últimos ~3 meses**, se descarta y se
   busca otra. Pasado ese plazo solo vuelve si la primera voló y pasa las puertas de `§4.4-REPETIR`
   (`global §2.0b-VENTANA`: la referencia es lo más visible de un meme, por eso su ventana es más larga
   que las 3 publicaciones de las demás listas; 98 días es el único hueco medido que funcionó).
...
**🔴🔴 8. AL REUTILIZAR UN RECURSO VIEJO, EL FORMATEADO SE AUDITA ENTERO, NO SOLO EL CONTENIDO (Iker, 2026-09-17).** `/perfil/` se regaló con su maquetación de mayo: tope de lectura de 720px en 59 de 69 párrafos, maquetas de LinkedIn pegadas a la izquierda, H1 del gate y de la guía a 3 líneas y bloques de 4-5. Iker: *"no tendría que estar yo revisando recurso por recurso a ver si lo has adaptado bien"*. **Antes de dar un recurso reutilizado por bueno, se miden en 1440 y 360, contra la norma medida en los 10 recursos:** H1 en 2 líneas (gate y guía a 1440; gate también a 360), entrada del gate en 3 y 4, ningún texto de más de 3 líneas salvo las maquetas que imitan un texto real, cero párrafos estrechos, cero viudas, maquetas topadas pero centradas. `validar-recurso.py` caza ya el tope heredado (check 6). **Obligatorio en todo recurso que se toque, nuevo o reutilizado, y su salida va en la entrega:** `node tools/qa-formato.cjs 8788 <slug>` (en neety-resources, con el servidor local levantado) tiene que decir `formateado en norma`. El 17/09 se auditaron los 11 recursos con esa herramienta y todos quedaron en norma. De paso salieron las lecciones que se repiten: topes heredados en `.container p`, `.lede`, `.teaser p` y `.cta-final p`; H1 a 1 o 3 líneas; cifras sin fuente (11x, 71%, 92%, 40+ equipos…) que se quitaron; formularios que prometían un correo que no se manda; secciones de apoyo numeradas (van con ●); y bloques gemelos con distinto número de líneas. Lo que la herramienta no mide y se mira a ojo: celdas de tabla, texto centrado y el móvil de la guía.
...
>
> **LA REGLA: el vehículo entra en la ley de variedad y no se repite en las 3 historias siguientes de esa cuenta** (`global §2.0b-VENTANA`). Si otra cuenta lo acaba de usar (menos de 7 días), se dice en la entrega: el mismo lector ve los tres perfiles, pero no se prohíbe.
>
> **VEHÍCULOS YA USADOS** (frenan solo a su cuenta y caducan, `global §2.0b-VENTANA`): el instituto y la infancia (×4 hasta agosto: el más visto, así que conviene espaciarlo aunque esté libre) · la primera venta de crío (Iker 18/08) · el Excel del cliente en una reunión (Iker 13/08).
...
- **⛔ REGLA DURA (Iker, 2026-07-27) — UN SOLO MAPA Y UN SOLO "LOS 10" POR SEMANA, CONTANDO LAS TRES CUENTAS.** En una misma semana, **solo una cuenta** publica mapa y **solo otra cuenta** publica "Los 10". La tercera cuenta se queda **sin peloteo** esa semana (hasta que exista el tercer formato de peloteo, §8.1b). Esto ANULA lo que decía antes esta línea ("sí pueden coincidir dos cuentas en el mismo pilar la misma semana"), que era falso y nos costó caro.
  ...
  - Y como siempre: regiones distintas y personas distintas, y **el concepto no se repite en los 3 peloteos siguientes de esa cuenta** (`global §2.0b-VENTANA`).
